...
- **Groq + Llama-3**
  - Uses `llama-3.3-70b-versatile` through Groq for **10x faster inference** and low-latency streaming UX.
- **FAISS**
...
- **PDF upload and indexing**
  ...
  - Embed with `all-MiniLM-L6-v2` by default
...
- **Embeddings**: HuggingFace `sentence-transformers/all-MiniLM-L6-v2` by default, configurable via env
- **Vector Store**: FAISS
...
If Groq retires a model, set `GROQ_MODEL` to any currently available chat model from your Groq account without changing the code.

If you want stronger multilingual retrieval quality, especially for Arabic-heavy documents, you can switch the embedding model to:
